...
export class Experience extends KeywordMixin(BaseListManager) {
  ...
  createItemHTML(index) {
    return `
      ...
      <input id="experienceList-title-${index}" class="title" placeholder="Cargo" required aria-required="true" aria-label="Cargo"/>
      <label for="experienceList-company-${index}">EMPRESA</label>
      <input id="experienceList-company-${index}" class="company" placeholder="Empresa" required aria-required="true" aria-label="Empresa"/>
      <label for="experienceList-startsAt-${index}">DATA DE INÍCIO</label>
      <input type="date" id="experienceList-startsAt-${index}" class="startsAt" required aria-required="true" aria-label="Data de início"/>
      <label for="experienceList-endsAt-${index}">DATA DE TÉRMINO</label>
      <input type="date" id="experienceList-endsAt-${index}" class="endsAt" aria-required="false" aria-label="Data de término"/>
      <div class="form-group">
        <label for="experienceList-currently-${index}">
          <input type="checkbox" id="experienceList-currently-${index}" aria-required="false" class="currently"/>
          Trabalha Atualmente?
        </label>
      </div>
      <label for="experienceList-description-${index}">DESCRIÇÃO</label>
      <textarea id="experienceList-description-${index}" class="description" placeholder="Descrição" required aria-required="true" aria-label="Descrição das atividades"></textarea>
    `;
  }
